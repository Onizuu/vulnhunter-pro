"""
Client Mistral AI pour génération de payloads et analyse
"""
...
try:
    ...
    logger.warning("⚠️  Bibliothèque mistralai non installée - Installez avec: pip install mistralai")
...
class ClientMistral:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not MISTRAL_AVAILABLE:
            logger.error("❌ Bibliothèque mistralai non disponible")
            self.disponible = False
            return
        
        self.api_key = api_key or os.getenv('MISTRAL_API_KEY')
        
        if not self.api_key or self.api_key == 'your-mistral-key-here':
            logger.warning("⚠️  Clé API Mistral non configurée - Mode sans IA activé")
            ...
            try:
                ...
                logger.warning(f"⚠️  Impossible d'initialiser Mistral: {str(e)}")
                self.disponible = False
        
        # Modèle Mistral recommandé (mistral-large-latest ou mistral-medium-latest)
    ...
    async def generer_completion(
        ...
        """
        Génère une completion avec Mistral AI
        ...
        if not self.disponible:
            logger.warning("Client Mistral non disponible")
            return None
...
- Prochaines étapes"""
    ...
    def set_modele(self, modele: str):
        """
        Change le modèle Mistral utilisé
        
        Args:
            modele: Nom du modèle (mistral-large-latest, mistral-medium-latest, etc.)
        """
        self.modele = modele
